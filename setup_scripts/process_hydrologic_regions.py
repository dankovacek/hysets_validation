...
bc_border_file = '../source_data/BC_border/BC_PROV_BOUNDARIES_LINES_500M.geojson'
...
def download_hydrologic_feature_data(pgs):
    save_directory = '/media/danbot/Samsung_T5/geospatial_data/WSC_data/NHN_feature_data/'
    # save_directory = '../processed_data/WSC_data/NHN_feature_data/'
...
bc_merged_filled.to_file(f'../processed_data/merged_basin_groups/BC_basin_region_groups_EPSG{dem_crs}.geojson', driver='GeoJSON')
print(f'   ...BC_basin_region_groups.geojson file created successfully.')

# save each row as a separate shape file
out_dir = '../processed_data/merged_basin_groups/split_groups/'
for code in bc_merged_filled.index.values:
    gdf = bc_merged_filled[bc_merged_filled.index == code].copy()
    gdf.to_file(f'{out_dir}{code}_{dem_crs}.geojson', driver='GeoJSON')
print(f'   ...saved individual polygons seperately at {out_dir}')
